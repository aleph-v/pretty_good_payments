...
contract DeployTestnet is Script {
    ...
    function _generateUnifiedConfig(
        address entrypoint,
        address registry,
        string memory rpcUrl,
        uint256 chainId
    ...
    function _configHeader(uint256 chainId) internal view returns (string memory) {
        return string.concat(
            ...
            "# Generated at block: ", vm.toString(block.number), "\n",
            "#\n",
    ...
    function _contractsSection(address entrypoint, address registry) internal view returns (string memory) {
        return string.concat(
            ...
            "deposits = \"", vm.toString(entrypoint), "\"\n",  // Same contract for now
            "transaction_registry = \"", vm.toString(registry), "\"\n\n"
        );
    }
    ...
    function _writeVerificationScript(
        ...
        script = string.concat(
            ...
            "  --chain-id $CHAIN_ID \\\n",
            "  --constructor-args $(cast abi-encode \"constructor(address,string,string)\" ", vm.toString(token), " \"PGP Testnet Vault\" \"vtPGP\") \\\n",
            "  --etherscan-api-key $ETHERSCAN_API_KEY \\\n",
            "  --watch\n\n"
        );
    ...
    function _logVerificationCommands(
        ...
        console.log(string.concat(
            ...
            " test/mocks/TestnetERC20.sol:TestnetERC20 --chain-id ", cid,
            " --constructor-args $(cast abi-encode 'constructor(string,string)' 'PGP Testnet Token' 'tPGP')",
            " --etherscan-api-key $ETHERSCAN_API_KEY"
        ));
        ...
        console.log(string.concat(
            ...
            " test/mocks/TestnetERC4626.sol:TestnetERC4626 --chain-id ", cid,
            " --constructor-args $(cast abi-encode 'constructor(address,string,string)' ",
            vm.toString(token), " 'PGP Testnet Vault' 'vtPGP')",
            " --etherscan-api-key $ETHERSCAN_API_KEY"
        ));
        ...
        console.log(string.concat(
            "forge verify-contract ", vm.toString(transferVerifier),
            " test/mocks/FakeZk.sol:FakeZK --chain-id ", cid,
            " --etherscan-api-key $ETHERSCAN_API_KEY"
        ));
        ...
        console.log(string.concat(
            "forge verify-contract ", vm.toString(updateVerifier),
            " test/mocks/FakeZk.sol:FakeZK --chain-id ", cid,
            " --etherscan-api-key $ETHERSCAN_API_KEY"
        ));
        ...
        console.log(string.concat(
            "forge verify-contract ", vm.toString(registry),
            " src/TransactionRegistry.sol:TransactionRegistry --chain-id ", cid,
            " --etherscan-api-key $ETHERSCAN_API_KEY"
        ));
        ...
        console.log(string.concat(
            "forge verify-contract ", vm.toString(entrypoint),
            " src/Entrypoint.sol:Entrypoint --chain-id ", cid,
            " --constructor-args $(cast abi-encode 'constructor(bytes32,address,address,address,address)' ",
            vm.toString(genesis), " ", vm.toString(yieldRouter), " ",
            vm.toString(updateVerifier), " ", vm.toString(transferVerifier), " ",
            vm.toString(registry), ")",
            " --etherscan-api-key $ETHERSCAN_API_KEY"
        ));
        ...
        console.log(string.concat(
            "forge verify-contract ", vm.toString(yieldRouter),
            " src/YieldRouter.sol:YieldRouter --chain-id ", cid,
            " --constructor-args $(cast abi-encode 'constructor(uint256,uint256,address,address[])' ",
            vm.toString(PERIOD_LENGTH), " ", vm.toString(getEpochsPerPeriod()), " ",
            vm.toString(entrypoint), " '[", vm.toString(token), "]')",
            " --etherscan-api-key $ETHERSCAN_API_KEY"
        ));
    }
}
